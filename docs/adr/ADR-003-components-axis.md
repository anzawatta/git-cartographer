...
cartographer agent / Explorer はリポジトリ構造を毎回フルスキャンしており、
...
- **忘れられる構造（PRINCIPLE 1）**: 多くのリポジトリは `.cartographer.toml` を置かなくても動く
- **同梱 toml をリポジトリ側に配る方式は却下**: リポジトリ側に余計なファイルを増やす運用が「忘れられる構造」を破壊する
...
#### 2026-04-29: scan_dirs ネストパス対応

- `scan_dirs` にネストしたパス（例: `"src/modules"`, `"packages/shared"`）を指定可能にした。
- 深いパスと浅いパスが重複する場合（例: `"src"` と `"src/modules"` の両方を指定）は、
  **深いパスを優先**する。これにより `src/modules/foo/bar.ts` は `src` ではなく `src/modules` に属する。
- 任意の深さ（`"a/b/c"` のような3階層以上）のパスも有効。
...
**C. リポジトリ側に同梱 `.cartographer.toml` を必須化する**
- 理由: 「忘れられる構造」を破壊する。多くのリポジトリでノーアクションで動く設計を優先する
