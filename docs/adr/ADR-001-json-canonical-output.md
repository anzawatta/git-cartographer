...
Markdown ファイル（`stable.md`, `co-change.md`, `hotspot.md`）は引き続き並行生成する。
...
- 計算不能なフィールド（`decay_factor`, `trend` 等）は `null` とする
- 将来フィールドを追加する際は EARS-001 を更新する

### stability_score の実装（F2 decay model）

`stability_score` は当初 "計算不能" として `null` 固定だったが、REQ-S004 実装により算出可能となった。

採用式: `stability_score = 1.0 - 0.5^(commits_since_last_change / halflife_stable)`
- `halflife_stable` デフォルト: 200 コミット（200コミット前に最終変更されたファイルがスコア≈0.5）
- 算出には `commits_since_last_change_bulk()` を使用（単一 `git log` パスで全 stable ファイルを一括処理）
- git 障害またはスキャン範囲内に見つからないファイルは `null` フォールバック
