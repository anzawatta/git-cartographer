...
## AST symbol digest 注入（新規）

### イベント駆動条件

4. REQ-E004: PreToolUse Hook が発火し、`tool_name == "Read"` かつ `agent_type` が REQ-W003 の allowlist に含まれるとき、System は以下の手順で AST symbol digest コンテキストを生成し、フック応答の additionalContext に追加しなければならない:

   a. **ward 解決**: REQ-E003 の手順 a と同じ（`.cartographer_state` からの output_dir 解決）

   b. **ast-digest.json 参照**: `{output_dir}/ast-digest.json` を読み込み、`files[].path` 一覧を取得する

   c. **ファイル判定**: `tool_input.file_path` の `.cartographer_state` ディレクトリからの相対パスが `files[].path` のいずれかと一致する場合、そのエントリを取得する。一致しない場合は注入をスキップする

   d. **skip 条件**: エントリの `parse_status` が `"ok"` でない場合、または `symbols` が空の場合は注入をスキップする

   e. **フォーマット**: シンボルリストを Markdown テーブルで整形する:
      ```
      ## Symbol Digest: {rel_path}

      | Name | Kind | Lines |
      |------|------|-------|
      | `Cartographer` | class | 25–180 |
      | `Cartographer.run` | method | 42–95 |
      ```
      フッター行として `_このシンボルリストは git-cartographer の ast-digest.json から自動生成されています。_` を追加する

### 敵対条件

5. REQ-W005: System は、`ast-digest.json` が存在しない場合、注入をスキップしなければならない。その際、`stderr` に一行 `"warning: ast-digest.json not found at {path}; symbol resolution disabled. → Fix: run cartographer first"` を出力しなければならない（EARS-004 [ADV] GZ-9 準拠）

6. REQ-W006: System は、`parse_status` が `"ok"` でない場合、または `symbols` が空の場合、AST コンテキストを注入してはならない（`"skipped_language"` のファイルに対してサイレントスキップ）
